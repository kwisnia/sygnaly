...
def low_pass_filter(filter_order, cutoff_frequency, sample_rate, window_type):
    if window_type == 1:
        ...
        window = [rectangular_window(filter_order) for _ in filter_order]
...
def high_pass_filter(filter_order, cutoff_frequency, sample_rate, window_type):
    return np.array([x * -1 ** i for i, x in enumerate(low_pass_filter(filter_order, cutoff_frequency, sample_rate, window_type))])
