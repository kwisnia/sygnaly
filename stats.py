...
def mean_square_error(original_signal, reconstructed_signal):
    return sum([(original_signal.values[i] - reconstructed_signal.values[i]) ** 2 for i in range(len(original_signal.samples))]) / len(original_signal.values)


def signal_to_noise_ratio(original_signal, reconstructed_signal):
    return 10 * math.log(sum((original_signal.values ** 2) / sum([(original_signal.values[i] - reconstructed_signal.values[i]) ** 2 for i in range(len(original_signal.samples))])), 10)
...
def maximum_difference(original_signal, reconstructed_signal):
    return max([original_signal.values[i] - reconstructed_signal.values[i] for i in range(len(original_signal.samples))])